...
VIotaModule::VIotaModule(VerilatedContext* _vcontextp__, const char* _vcname__)
    : VerilatedModel{*_vcontextp__}
    , vlSymsp{new VIotaModule__Syms(contextp(), _vcname__, this)}
...
VIotaModule::VIotaModule(const char* _vcname__)
    : VIotaModule(Verilated::threadContextp(), _vcname__)
{
}
...
void VIotaModule___024root___eval_initial(VIotaModule___024root* vlSelf);
void VIotaModule___024root___eval_settle(VIotaModule___024root* vlSelf);
...
#endif  // VL_DEBUG
void VIotaModule___024root___final(VIotaModule___024root* vlSelf);

static void _eval_initial_loop(VIotaModule__Syms* __restrict vlSymsp) {
    vlSymsp->__Vm_didInit = true;
    VIotaModule___024root___eval_initial(&(vlSymsp->TOP));
    // Evaluate till stable
    vlSymsp->__Vm_activity = true;
    do {
        VL_DEBUG_IF(VL_DBG_MSGF("+ Initial loop\n"););
        VIotaModule___024root___eval_settle(&(vlSymsp->TOP));
        VIotaModule___024root___eval(&(vlSymsp->TOP));
    } while (0);
}

void VIotaModule::eval_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+++++TOP Evaluate VIotaModule::eval_step\n"); );
#ifdef VL_DEBUG
    // Debug assertions
    VIotaModule___024root___eval_debug_assertions(&(vlSymsp->TOP));
#endif  // VL_DEBUG
    // Initialize
...
const char* VIotaModule::name() const {
    return vlSymsp->name();
}
...
VL_ATTR_COLD void VIotaModule::final() {
    VIotaModule___024root___final(&(vlSymsp->TOP));
}

//============================================================
// Implementations of abstract methods from VerilatedModel

const char* VIotaModule::hierName() const { return vlSymsp->name(); }
const char* VIotaModule::modelName() const { return "VIotaModule"; }
unsigned VIotaModule::threads() const { return 1; }
std::unique_ptr<VerilatedTraceConfig> VIotaModule::traceConfig() const {
...
void VIotaModule___024root__trace_init_top(VIotaModule___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    VIotaModule___024root__trace_init_top(vlSelf, tracep);
...
VL_ATTR_COLD void VIotaModule::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'VIotaModule::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    VIotaModule___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
