...
#ifndef VERILATED_VCHIMODULE_H_
#define VERILATED_VCHIMODULE_H_  // guard

#include "verilated.h"
#include "verilated_cov.h"

class VChiModule__Syms;
class VChiModule___024root;
class VerilatedVcdC;

// This class is the main interface to the Verilated model
class VChiModule VL_NOT_FINAL : public VerilatedModel {
  private:
    // Symbol table holding complete model state (owned by this class)
    VChiModule__Syms* const vlSymsp;
  ...
  public:
    ...
    VChiModule___024root* const rootp;
    ...
    explicit VChiModule(VerilatedContext* contextp, const char* name = "TOP");
    explicit VChiModule(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~VChiModule();
  private:
    VL_UNCOPYABLE(VChiModule);  ///< Copying not allowed
