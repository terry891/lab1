...
class RhoPiModuleTests extends TestWithBackendSelect with ChiselScalatestTester {
  behavior of "RhoPiModule"
  
  it should "do the RhoPi function" in {
    test(new RhoPiModule()).withAnnotations(
      simAnnos
    ) { c =>
      ...
      for (iter <- 0 to 100) {
        ...
        //PeekPoke testing
        for (i <- 0 until 25)
          c.io.state_i(i).poke(st(i))
        c.clock.step(1)
        for (j <- 0 until 25)
          c.io.state_o(j).expect(state_temp(j))
      }
    }
  }
}
